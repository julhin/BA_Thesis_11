...
\label{req_roles}
CoCoME defines six different roles. Five are defined in the CoCoME tech report \cite{CoCoMETechReport}. These are the \textit{Customer}, the \textit{Cashier}, the \textit{StoreManager}, the \textit{Enterprise manager}, \textit{Stockmanager}. The last role, the \textit{Admin} only appears in the code. There is no separate description of the roles. All the responsibilities are derived from the use cases described in the documentation and/or the implementation. The key informations we were looking are the tasks that a role performs in the context of CoCoME and which data is provided/required by the role. 
\begin{itemize}
...
\end{itemize}
Since the hybrid cloud based variant is examined by us, all roles provide account data, because each role ,in the hybrid cloud-based variant, needs an account in CoCoME. 
\subsubsection{R5: access rights}
%TODO Admin in der Matrix hinzufügen !!!!!!!!!! %TODO
To define the access rights, we use the finer grained, high level from proposed by Evered and Bögeholz \cite{CaseStudyAndAccessrigths} in contrast to the well known read/write semantics for access control. We tried not to rely on a read/write semantic to allow a more detailed description of the access rights. Therefore, as described in \nameref{ch:method}, we defined access to the various data  within a component for each component and for each role. We defined four different privilege level for the access to data. In the following enumeration, we present the defined access rights. We used the  elements from the previous requirements R2 (\autoref{req_uc}), R3 (\autoref{req_data}) and R4 (\autoref{req_roles}) to define the access rights for CoCoME. The access to data decreases with the ascending numbers.
\begin{enumerate}
...
The last requirement defines the type of data processing. For each component, it is defined how the data from each class is processed in this component. All in all, we defined four types of data processing in CoCoME.
\begin{itemize}
...
\subsection{P3: Analysis of the current state for CoCoME}
...
\end{table}
\end{landscape}
\section{Define system extensions}
%TODO Admin role genauer definieren
\section{Evaluation of the defined access rights}
